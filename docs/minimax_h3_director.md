...
- **Image Inpaint mode:** accepts exactly one enabled image reference from the Director timeline, rejects video/audio references, and forces the visible Director frame-count contract to `1`. Native H3 execution still uses its valid five-frame latent minimum, then decodes and saves only frame 0. Enable the Director's optional `internal_execute` input with the FL2VA model, CLIP, VAE and seed to run this self-contained path; it persists an allowlisted postprocess recipe and saves workflow/prompt metadata by default (`save_workflow` may be explicitly disabled in `timeline_data.internal_execution.save`). No bridge nodes, generated-image connector, or auto-created graph nodes are used.
