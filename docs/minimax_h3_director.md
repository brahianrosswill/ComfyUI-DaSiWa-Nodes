...
- **Decoder precedence:** `preview_tiny_vae` widget (a `models/vae_approx` filename combo, e.g. `taeh3.safetensors` — the special H3 tiny decoder that core's `VAELoader` cannot build) > `preview_vae` input socket (full-quality `vae.decode()`) > core latent previewer > latent-to-RGB fallback. The tiny-VAE path clamps output before uint8 because taeh3 output is not [0,1]-guaranteed.
