...
- **08-29:** **PDD/Acc LoRA head-bank guard:** when a PDD LoRA (`pdd_num_steps`) is applied to a single-head H3 model, the incompatible `final_layer` head-bank `set`/`bias` keys are skipped with a warning (instead of crashing core at `comfy/lora.py`), so the LoRA adapters still apply. The loader detects the mismatch only on positive evidence (bank width and model width both readable and different); a genuine PDD model whose width matches is untouched. **Recommended fix:** pair PDD LoRAs with a PDD model (`final_layer.video_out [3072,5376]`). Bugfix — no version bump.
