...
function install(node) {
  ...
  const render = () => {
    ...
    const addDropdown = (label, value, options, onChange, withAspectSwatches = false, withHints = false) => { const field = document.createElement("div"); field.className = "ds-h3-res-field"; field.textContent = label; const control = document.createElement("span"); control.className = "ds-h3-res-control"; const select = document.createElement("select"); select.className = "ds-h3-res-select"; select.setAttribute("aria-hidden", "true"); options.forEach(([id, text]) => { const option = document.createElement("option"); option.value = id; option.textContent = text; select.append(option); }); select.value = value; select.oninput = event => { onChange(event.target.value); syncButton(); syncMenu(); }; const button = document.createElement("button"); button.type = "button"; button.className = "ds-h3-res-btn"; button.setAttribute("aria-haspopup", "listbox"); button.setAttribute("aria-expanded", "false"); const labelSpan = document.createElement("span"); labelSpan.className = "ds-h3-res-label"; const caret = document.createElement("span"); caret.className = "ds-h3-res-caret"; caret.textContent = "▾"; button.append(labelSpan, caret); const menu = document.createElement("div"); menu.className = "ds-h3-res-menu"; menu.setAttribute("role", "listbox"); const syncButton = () => { const current = options.find(([id]) => id === select.value); labelSpan.textContent = current ? current[1] : select.value; const swatch = withAspectSwatches ? aspectSwatch(select.value) : null; const existing = button.querySelector(".ds-h3-res-swatch"); if (existing) existing.remove(); if (swatch) button.insertBefore(swatch, labelSpan); button.title = withHints ? inputScalingHint(select.value) : ""; }; const syncMenu = () => menu.querySelectorAll(".ds-h3-res-item").forEach(item => item.classList.toggle("active", item.dataset.id === select.value)); options.forEach(([id, text]) => { const item = document.createElement("button"); item.type = "button"; item.className = "ds-h3-res-item"; item.setAttribute("role", "option"); item.dataset.id = id; if (withAspectSwatches) { const swatch = aspectSwatch(id); if (swatch) item.append(swatch); } const itemLabel = document.createElement("span"); itemLabel.className = "ds-h3-res-item-label"; itemLabel.textContent = text; item.append(itemLabel); if (withHints) item.title = inputScalingHint(id); item.onclick = () => { select.value = id; onChange(id); syncButton(); syncMenu(); closeAllMenus(); button.setAttribute("aria-expanded", "false"); }; menu.append(item); }); button.onclick = () => { const willOpen = !menu.classList.contains("open"); closeAllMenus(); menu.classList.toggle("open", willOpen); button.setAttribute("aria-expanded", String(willOpen)); }; syncButton(); syncMenu(); control.append(select, button, menu); field.append(control); resolutionPanel.append(field); };
    ...
    addDropdown("ASPECT", settings.aspect, ASPECT_OPTIONS, aspect => updateResolution({ aspect }), true);
    addDropdown("RESOLUTION", settings.resolution, [["auto", "Auto · short side 768 px"], ...Object.keys(RESOLUTION_PRESETS).map(name => [name, name]), ["custom", "CUSTOM"]], resolution => updateResolution({ resolution }));
    addDropdown("INPUT SCALING", settings.input_scaling, [["Off", "Off"], ["Auto", "Auto · short edge 2048 px"], ["Target", "Target · Selected Aspect & Resolution"], ["Fit", "Fit"], ["Fill and crop", "Fill and crop"], ["Fit and pad", "Fit and pad"], ["Long side with divisible crop", "Long side with divisible crop"]], input_scaling => updateResolution({ input_scaling }), false, true);
